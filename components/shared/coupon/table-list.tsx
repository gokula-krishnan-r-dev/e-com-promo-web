...
const TableList = () => {
  ...
  const printSelectedRow = async () => {
    // Array of selected coupon IDs (adjust this dynamically as needed)
    const ids: string[] = [
      "6729a577b0702864d47aa09c",
      "6729a55db0702864d47aa07e",
    ];

    // Log selected rows (optional for debugging)
    console.log(selectedRow, "selectedRow");

    // Show loading toast
    toast.loading("Generating PDFs, please wait...");

    // Function to generate and download PDF for each coupon
    try {
      for (let i = 0; i < ids.length; i++) {
        const id = ids[i];

        // Navigate to the coupon view page
        await router.push(`/coupon/view/${id}`);

        // Wait for the page to load
        await new Promise((resolve) => setTimeout(resolve, 5000)); // Adjust based on page load time
        ...
        if (element) {
          const options = {
            margin: 0.5,
            filename: `coupon_${id}.pdf`,
            image: { type: "jpeg", quality: 1 },
            html2canvas: { scale: 2 },
            jsPDF: { unit: "in", format: "a4", orientation: "portrait" },
          };

          // Additional PDF customization can be done here
          html2pdf().set(options).from(element).save();

          toast.success(`PDF download started for coupon ${id}...`);
        } else {
          console.error(
            `Element with ID 'coupon-content' not found for coupon ${id}.`
          );
          toast.error(
            `Failed to download PDF for coupon ${id}. Please try again later.`
          );
        }
      }
    } catch (error) {
      ...
      // Hide loading toast once the process is complete
      toast.dismiss();
      router.push("/coupon");
    }
  };
