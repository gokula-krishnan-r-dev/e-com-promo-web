  ...
  const tabData = [
    {
      ...
      filter: "",
      content: (
        <DiscountTable data={discounts} columns={DiscountColumnstable} />
      ),
    },
    {
    ...
    try {
      ...
      const deletePromises = ListIds.map(async (id) => {
        const response = await fetch(
          search === "first_order_discount"
            ? `https://e-com-promo-api-57xi.vercel.app/api/v1/discounts/firstorder/${id}`
  ...
  return (
    <div className="px-6 py-3">
      ...
      <div className="">
        <DynamicTabs
          router={router}
          defaultValue={search ? search : "discount_list"}
          tabs={tabData}
        />
      </div>
    </div>
