...
import { CreditTable } from "./table/credit-table";
import { toast } from "sonner";

const CreditTableList = ({ edit }: any) => {
  const { credits, selectedRow, refetch, setRowSelection } = useCreditContext();
    ...
    try {
      ...
      const deletePromises = ListIds.map(async (id) => {
        const response = await fetch(
          `https://e-com-promo-api-57xi.vercel.app/api/v1/credits/${id}`,
          {
            method: "DELETE",
          }
        );
