...
const DiscountForm: React.FC<any> = ({ defaultValue, method }: any) => {
  const router = useRouter();
  const { refetch } = useDiscountContext();

  const { data, isLoading, mutate, error } = useMutation(
    `/discounts/${method === "PUT" ? defaultValue._id : ""}`,
    {
  ...
  const validate = (): boolean => {
    const { error } = discountFormSchema.validate(finalList, {
      abortEarly: false,
    });
